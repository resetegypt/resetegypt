...
export function ClinicalFormPage() {
  ...
  const { data: recordData, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGet<{ medicalRecord: MedicalRecord | null }>(`/patients/${patientId}/medical-record`),
    enabled: !!patientId,
  });
  ...
  const finalizeMut = useMutation({
    mutationFn: () =>
      api<{ medicalRecord: MedicalRecord }>(`/patients/${patientId}/medical-record/finalize`, {
        method: 'POST',
      }),
    onSuccess: (data) => {
      qc.setQueryData(['medical-record', patientId], { medicalRecord: data.medicalRecord });
            ...
            >
              <div className="flex items-center gap-2">
                {isFinalized ? (
                  ...
                  <span>
                    ⚠️ {t('clinical.saveError', 'Erreur')} : {saveError}
                  </span>
                ) : upsertMut.isPending ? (
                  <>
                  ...
                  <>
                    ...
                    <span>
                      {t('clinical.savedAt', 'Sauvegardé')} {timeAgo(savedAt)}
                    </span>
                  </>
                ) : (
                  <>
          ...
          {!canEdit && (
            <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border bg-bg-secondary/40 mb-5 text-sm text-text-secondary">
              ...
              <span>
                {t(
                  'clinical.readOnly',
                  'Mode consultation — seul le médecin peut modifier cette fiche.',
                )}
              </span>
            </div>
          )}
          ...
          {isLoading && !record ? (
            ...
            <div className="space-y-5">
              <Section
                id="addiction"
                icon="🚬"
                title="L'addiction en détail"
                description="Comprendre quand et comment a démarré la dépendance."
              >
                <Grid>
                  <NumberField
                    label="Années d'addiction"
                    suffix="ans"
                    value={form.yearsOfAddiction}
                    onChange={(v) => update('yearsOfAddiction', v)}
                    disabled={!canEdit}
                  />
                  <DateField
                  ...
                  />
                  <TextField
                    label="Quantité quotidienne"
                    placeholder="ex: 1 paquet/jour"
                    value={form.dailyQuantity}
                    onChange={(v) => update('dailyQuantity', v)}
                    disabled={!canEdit}
                  />
                  <NumberField
                    label="Coût estimé / mois"
                    suffix="EGP"
                    value={form.estimatedCostPerMonth as number | null}
                    onChange={(v) =>
                      update(
                        'estimatedCostPerMonth',
                        v as unknown as MedicalRecord['estimatedCostPerMonth'],
                      )
                    }
                    disabled={!canEdit}
                  />
              ...
              <Section
                id="context"
                icon="🎯"
                title="Déclencheurs & contexte"
                description="Qu'est-ce qui pousse le patient à consommer ?"
              >
                <FieldLabel label="Déclencheurs (multi)">
                  <div className="flex flex-wrap gap-1.5">
                    {TRIGGERS.map((trg) => (
                      <Chip
                        key={trg}
                        active={(form.triggers ?? []).includes(trg)}
                        onClick={() => toggleArrayValue('triggers', trg)}
                        disabled={!canEdit}
                      >
                ...
                <TextareaField
                  label="Moments précis de consommation"
                  placeholder="ex: après chaque repas, en sortie le weekend…"
                  value={form.consumptionMoments}
                  onChange={(v) => update('consumptionMoments', v)}
                  disabled={!canEdit}
                />
                <Grid>
                  <TextareaField
                    label="Contexte social"
                    placeholder="ex: conjoint fume aussi, groupe d'amis…"
                    value={form.socialContext}
                    onChange={(v) => update('socialContext', v)}
                    disabled={!canEdit}
                  />
                  <TextareaField
                    label="Contexte professionnel"
                    placeholder="ex: métier stressant, pauses cigarette…"
                    value={form.workContext}
                    onChange={(v) => update('workContext', v)}
                    disabled={!canEdit}
                  />
                </Grid>
              </Section>
              ...
              <Section
                id="history"
                icon="⏪"
                title="Tentatives précédentes"
                description="Ce qui a marché, ce qui n'a pas marché."
              >
                <Grid>
                  <NumberField
                  ...
                  />
                  <TextField
                    label="Plus longue période d'arrêt"
                    placeholder="ex: 6 mois en 2022"
                    value={form.longestQuit}
                    onChange={(v) => update('longestQuit', v)}
                    disabled={!canEdit}
                  />
                </Grid>
                <TextareaField
                  label="Raisons de la rechute"
                  placeholder="ex: deuil familial, retour au boulot…"
                  value={form.relapseReasons}
                  onChange={(v) => update('relapseReasons', v)}
                  disabled={!canEdit}
                />
                <FieldLabel label="Méthodes déjà essayées (multi)">
                  <div className="flex flex-wrap gap-1.5">
                    {METHODS.map((m) => (
                      <Chip
                        key={m}
                        active={(form.methodsTried ?? []).includes(m)}
                        onClick={() => toggleArrayValue('methodsTried', m)}
                        disabled={!canEdit}
                      >
              ...
              <Section
                id="scores"
                icon="📊"
                title="Évaluations 0-10"
                description="Auto-évaluation du patient de 0 (rien) à 10 (extrême)."
              >
                {[
                  ...
                  <SliderField
                    key={s.key}
                    label={s.label}
                    value={form[s.key as keyof MedicalRecord] as number | null}
                    onChange={(v) => update(s.key as keyof MedicalRecord, v as never)}
                    disabled={!canEdit}
                  />
                ))}
              </Section>
              ...
              <Section
                ...
                description="Mesures de base avant le protocole laser."
              >
                <Grid>
                  <NumberField
                    label="Poids"
              ...
              <Section
                ...
                description="À vérifier obligatoirement avant tout traitement."
                tone="warning"
              >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {CONTRAINDICATIONS.map((c) => (
                      <Chip
                        ...
                        onClick={() => toggleArrayValue('contraindications', c)}
                        disabled={!canEdit}
                      >
                ...
                <TextareaField
                  ...
                  onChange={(v) => update('contraindicationNotes', v)}
                  disabled={!canEdit}
                />
              </Section>
              ...
              <Section
                id="standardized"
                icon="📋"
                title="Scores standardisés"
                description="Selon le type d'addiction."
              >
                <Grid>
                  {patient.primaryAddiction === 'TOBACCO' && (
                    <NumberField
                      label="Fagerström (0-10)"
                      min={0}
                      max={10}
                      value={form.fagerstromScore}
                      onChange={(v) => update('fagerstromScore', v)}
                      disabled={!canEdit}
                    />
                  )}
                  {patient.primaryAddiction === 'ALCOHOL' && (
                    <NumberField
                      ...
                      min={0}
                      max={40}
                      value={form.auditScore}
                      onChange={(v) => update('auditScore', v)}
                      disabled={!canEdit}
                    />
                  )}
                  {patient.primaryAddiction === 'DRUGS' && (
                    <NumberField
                      ...
                      min={0}
                      max={44}
                      value={form.duditScore}
                      onChange={(v) => update('duditScore', v)}
                      disabled={!canEdit}
                    />
                  )}
                  {patient.primaryAddiction === 'SUGAR' && (
                    <NumberField
                      ...
                      min={0}
                      max={11}
                      value={form.yfasScore}
                      onChange={(v) => update('yfasScore', v)}
                      disabled={!canEdit}
                    />
                  )}
                  <NumberField
                    label="HAD-Anxiété (0-21)"
                    min={0}
                    max={21}
                    value={form.hadAnxietyScore}
                    onChange={(v) => update('hadAnxietyScore', v)}
                    disabled={!canEdit}
                  />
                  <NumberField
                    label="HAD-Dépression (0-21)"
                    min={0}
                    max={21}
                    value={form.hadDepressionScore}
                    onChange={(v) => update('hadDepressionScore', v)}
                    disabled={!canEdit}
                  />
                </Grid>
              </Section>
              ...
              <Section id="protocol" icon="💎" title="Protocole thérapeutique">
                <TextareaField
                  label="Points d'auriculothérapie"
                  placeholder="ex: Shen Men, Estomac, Foie…"
                  value={form.auricularPoints}
                  onChange={(v) => update('auricularPoints', v)}
                  disabled={!canEdit}
                />
                <Grid>
                  <NumberField
                    label="Durée du laser"
                    suffix="min"
                    value={form.laserDuration}
                    onChange={(v) => update('laserDuration', v)}
                    disabled={!canEdit}
                  />
                  <TextField
                    label="Fréquence des séances"
                  ...
                  />
                  <NumberField
                    label="Nb estimé de séances"
                    value={form.estimatedSessions}
                    onChange={(v) => update('estimatedSessions', v)}
                    disabled={!canEdit}
                  />
                </Grid>
              </Section>
              ...
              <Section id="goals" icon="🎯" title="Objectifs & plan">
                <TextareaField
                  ...
                  placeholder="ex: arrêter complètement avant juin"
                  value={form.patientGoals}
                  onChange={(v) => update('patientGoals', v)}
                  disabled={!canEdit}
                />
                <TextareaField
              ...
              {isPractitioner && (
                <Section
                  ...
                  icon="🔒"
                  title="Notes privées"
                  description="Visibles uniquement par les médecins."
                  tone="warning"
                >
                  <TextareaField
                    label="Notes"
                    value={form.privateNotes}
                    onChange={(v) => update('privateNotes', v)}
                    rows={6}
                    disabled={!canEdit}
                  />
                </Section>
                ...
                <Card>
                  <CardContent className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    {isFinalized && (
                      <Button
                        variant="outline"
                        onClick={() => navigate(`/patients/${patientId}?tab=clinique`)}
                      >
                        OK
                      </Button>
                    )}
...
function Section({
  ...
  return (
    <section id={id} className="scroll-mt-6">
      <Card>
        <div
          className={`px-5 py-4 border-b border-border-light ${tone === 'warning' ? 'bg-warning-light/40' : ''}`}
        >
  ...
  return (
    <FieldLabel label={label}>
      <textarea
        ...
        rows={rows}
        disabled={disabled}
        className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm placeholder:text-text-tertiary focus-visible:outline-none focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/15 disabled:opacity-50 disabled:cursor-not-allowed resize-none"
      />
    </FieldLabel>
  );
}
