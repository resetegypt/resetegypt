...
export async function practitionerMailRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/practitioner-mail/send', { bodyLimit: SEND_BODY_LIMIT }, async (req, reply) => {
    ...
    const html =
      d.bodyHtml ??
      `<pre style="font-family:inherit;white-space:pre-wrap">${d.bodyText
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')}</pre>`;
    ...
    if (!result.sent) {
      return reply
        .status(502)
        .send({ error: 'SendFailed', message: result.error, messageId: message.id });
    }
